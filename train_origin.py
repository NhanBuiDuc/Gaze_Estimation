        ...
        for user in userlist:
            ...
            for user_train in userlist_remain:
                ...
                for order in orders:
                    ...
                    # print(((f['label'][:]).T).shape)

                    train_data_temp = ((f['data'][:]).T).reshape(-1, 1, 260, 346)
                    train_label_temp = ((f['label'][:]).T).reshape(-1, 260, 346)
                    if flag == 1:
            ...
            for user_test in range(user, user + 1):
                for order in orders:
                    ...
                    test_data_temp = ((f['data'][:]).T).reshape(-1, 1, 260, 346)
                    test_label_temp = ((f['label'][:]).T).reshape(-1, 260, 346)
                    if flag == 1:
